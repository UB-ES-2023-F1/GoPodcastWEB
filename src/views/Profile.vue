      ...
      <div class="publish col-lg-10 col-md-9 col-sm-12 p-0">
        <div class="profile-content p-5">
          ...
          <div v-else class="profile-info" v-if="user">
            <img
              v-if="
                ...
                user.image_url !== null &&
                `${user.image_url}` !== 'data:image/jpeg;base64,'
              "
              :src="user.image_url"
              alt="profile"
            ...
            <div style="display: inline-block" class="ms-4">
              ...
              <div style="display: flex; flex-direction: column" v-else>
                <textarea
                  ...
                  type="text"
                  v-model="user.bio"
                  placeholder="Specify your bio here."
                ></textarea>
                <button class="bio-confirm" @click="updateBio()">
                  Confirm bio
                </button>
              </div>
            </div>
          ...
          <div class="my-podcasts" v-if="!searching">
            ...
            <PodcastList
              :podcastList="myPodcasts"
              v-if="myPodcasts.length > 0"
            />
            <div v-else>
              <h3 class="fw-lighter" v-if="isMyProfile">
                It looks like you don't have any podcast.
                <a
                  style="font-weight: bolder; cursor: pointer"
                  @click="this.$router.push({ name: 'PublishPodcast' })"
                  >Create your first one!!</a
                >
              </h3>
              <h3 class="fw-lighter" v-else>
                It looks like this user doesn't have any podcast.
              </h3>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    updateBio() {
      ...
      axios
        .put(path, formData, axiosConfig, headers)
        .then((res) => {
